// app/listings/favorites/page.tsx
...
export default async function FavoritesListingsPage({
  searchParams = {},
}: {
  ...
  const url = `${base}/api/listings/favorites?page=${page}&limit=${limit}`;
  ...
  if (!res.ok) {
    ...
    return (
      <main className="max-w-7xl mx-auto py-10 px-6">
        <p className="text-red-600">Lỗi tải danh sách bài đăng yêu thích.</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-7xl mx-auto py-10 px-6">
      <h1 className="text-xl text-black font-semibold mb-6">TIN ĐĂNG YÊU THÍCH</h1>

      {data.length === 0 ? (
        <p className="text-gray-600">Bạn chưa yêu thích bài đăng nào.</p>
      ) : (
