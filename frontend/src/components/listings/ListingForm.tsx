...
type ListingFormMode = "create" | "edit-user" | "edit-admin";
...
export default function ListingForm({ mode, listingId }: ListingFormProps) {
  ...
  const getHexForColor = (value: string, colors: ColorOption[]): string => {
    ...
    const isEdit = mode === "edit-user" || mode === "edit-admin";
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!res.ok) {
            console.error("Không tải được tin đăng:", res.status);
            return;
            }
            ...
            mileage_km: data.mileage_km ? String(data.mileage_km) : "",
            // backend đang lưu VND, mình convert về "triệu"
            price_vnd: data.price_vnd ? String(Math.round(data.price_vnd / 1_000_000)) : "",
            body_type: data.body_type || "",
            gearbox: data.gearbox || "so-tu-dong",
            fuel: data.fuel || "xang",
            seats: data.seats ? String(data.seats) : "",
            origin: data.origin || "trong-nuoc",
            color_ext: data.color_ext || "",
            color_int: data.color_int || "",
            video_url: data.video_url || "",
            // seller_name / phone / address_line: 
            // nếu bạn muốn luôn lấy từ user hiện tại, có thể giữ giá trị từ localStorage
            }));

            // Các ID để trigger load models/districts
            setBrandId(data.brand_id || "");
            setModelId(data.model_id || "");
            setProvinceId(data.province_id || "");
            setDistrictId(data.district_id || "");
        } catch (err) {
            console.error("Không tải được tin đăng:", err);
        }
    };

    loadListing();
  }, [isEdit, listingId, mode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    ? (mode === "edit-admin"
        ? `/api/admin/listings/${listingId}`  // sau này cho admin
        : `/api/listings/${listingId}`)       // user tự sửa tin của mình
    : "/api/listings";
