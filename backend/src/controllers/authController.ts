...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ success: true, message: "Email khôi phục đã được gửi" });
  } catch (err: any) {
    logger?.error(`Forgot password error: ${err.message}`);
    return res.status(500).json({ error: "Lỗi máy chủ khi gửi email khôi phục" });
  }
};
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || !newPassword) return res.status(400).json({ error: "Thiếu userId hoặc newPassword" });
